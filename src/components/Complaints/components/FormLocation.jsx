...
const FormLocation = props => {
    ...
    function ChildMap() {
        const center = useMemo(() => ({ lat: loc.latitude, lng: loc.longitude }), []);
    ...
    async function success(pos) {
        var crd = pos.coords;
        var data = await fetch('https://maps.googleapis.com/maps/api/geocode/json?latlng='+crd.latitude+','+crd.longitude+'&key='+process.env.REACT_APP_GOOGLE_MAPS_API_KEY);
        var datajson = await data.json();
        // console.log(datajson.results[0].address_components);
        var address = datajson.results[0].address_components;
        console.log(address);
        let country = null, state = null, city = null, lvl3 = null, locality = null, postalcode = null;
        ...
        setloc(prev=>({latitude:crd.latitude, longitude: crd.longitude, accuracy: crd.accuracy, postalcode:postalcode, city: city, state: state, country: country}))
    }
        ...
        if (navigator.geolocation) {
            ...
            .then(function (result) {
                if (result.state === "granted") {
                    ...
                    navigator.geolocation.getCurrentPosition(success, errors, mapoptions);
    ...
    return (
        <div className= "field">
            <label htmlFor="loc">Location<span className="required-star" style={{color:'red'}}>*</span></label><br />
            {/* <button type="button" onClick={()=>getLocation()} id="locbutton">Get current location</button> */}
            
            <Button variant="contained"onClick={()=>getLocation()} id="locbutton">Get current location
            </Button>
            {loc.latitude?<Map /> : <></>}
            <p id="getLoc">{!loc.latitude?<span>Please click the above button to give your current location</span>:<span>Latitude: {loc.latitude} <br /> Longitude: {loc.longitude} <br /> Accuracy: {loc.accuracy} meters </span>}</p>
        </div>
    )
}
